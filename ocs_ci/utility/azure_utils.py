...
class AZURETimeoutException(Exception):
    pass
...
class AZURE:
    ...
    def detach_volume(self, volume, node, timeout=120):
        ...
        for x in range(timeout):
            disk_state = self.get_disk_state(volume.name)
            logger.debug(
                "Volume id: %s has status: %s", volume.name,
                disk_state
            )
            if disk_state == 'Unattached':
                break
            if x == timeout - 1:
                raise AZURETimeoutException(
                    f"Reached timeout {timeout}s for volume to detach for "
                    f"volume ID: {volume.name}, Volume state: "
                    f"{disk_state}"
                )
            time.sleep(1)
